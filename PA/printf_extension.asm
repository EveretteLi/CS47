...
printf_format:
	...
	addi	$s0, $s0, 1
	lbu	$a0, 0($s0)		# load $s0 which is the input argument
	beq 	$a0, 'd', printf_int	
	beq	$a0, 's', printf_str
	# maybe add some error handleing from th input
printf_int: 
	lw	$a0, 0($t0)
	li	$v0, 1
	syscall				# print int at spot $a0
	j 	printf_last
printf_str:
	lw	$a0, 0($t0)
	li	$v0, 4
	syscall				# print string at spot $a0
	j 	printf_last	
printf_last:
